...
if (isset($_POST["submit"])) {
    if (updateData($_POST, $id, "supplier") > 0) {
        echo "<script>alert ('Berhasil')
        window.location.href = '../Barang/index.php';</script>";
    } else {
        echo "<script>alert('Gagal');</script>";
    }
}
$supplier = tampilData($id, "supplier");
    ...
    <div class="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md">
        ...
        <form action="#" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="supplier" name="supplier" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Masukkan nama supplier" value="<?= $supplier["0"]["nama"] ?>">
            </div>

            <!-- Kontak -->
            <div class="mb-4">
                <label for="kontak" class="block text-sm font-medium text-gray-700">Kontak</label>
                <input type="text" id="kontak" name="kontak" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Masukkan kontak supplier" value="<?= $supplier["0"]["kontak"] ?>">
            </div>

            <!-- Barang -->
            <div class="mb-4">
                <label for="barang" class="block text-sm font-medium text-gray-700">Barang</label>
                <textarea id="barang" name="barang" rows="4" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Masukkan daftar barang"><?= $supplier["0"]["barang"] ?></textarea>
            </div>
